...
pub fn testing() {
    ...
    let file = File::open("vec.bin").unwrap();
    let file = BufReader::new(file);
    let decoded: minivec = bincode::decode_from_reader(file, config).unwrap();
    ...
    let now = Instant::now();
    {
        ...
        println!("Buff: {:?}", buf);
        let mut file = File::create("./tester").unwrap();
        file.write_all(&buf).unwrap();
    }
    let elapsed = now.elapsed();
    println!("Write bytes elapsed: {:.5?}", elapsed.as_secs_f64());

    let mut page: [u8; 8] = [0x00; 8];
    let mut file = File::open("tester").unwrap();
    file.read_exact(&mut page).unwrap();
    println!("Contents: {:?}", page);
}
...
struct Info {
    term_number: u32,
    disk_length: usize,
    disk_byte_offest: usize,
}

impl MiniIndex {
    fn create_mini_index(&mut self, index: BTreeMap<String, Vec<(u32, u32)>>) {
        let total_len = index.len();
        let node_size = (total_len as f64).sqrt() as i64 + 1;
        let mut vec_test: Vec<String> = vec![];
        for (key, value) in index.iter() {
            vec_test.push(key.clone());
        }

        let vec_struct = minivec { vec: vec_test };

        //Save file:
        let now = Instant::now();
        let index_serial = vec_struct;
        let config = config::standard();
        let mut file = File::create("vec.bin").unwrap();
        encode_into_std_write(index_serial, &mut file, config).unwrap();
        let elapsed = now.elapsed();
        println!("Save mini Index elapsed: {:.5?}", elapsed.as_secs_f64());
    }
}
